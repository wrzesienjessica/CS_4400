...
sp_main: begin
    ...
    IF ip_airlineID is not NULL and ip_tail_num is not NULL THEN
		IF ip_airlineID IN (select airlineID from airline) and ip_locationID not in (select * from location) THEN
			IF ip_tail_num NOT IN (select tail_num from airplane where airlineID = ip_airlineID) THEN
				IF ip_speed > 0 AND ip_seat_capacity > 0 AND ip_speed is not NULL and ip_seat_capacity is not NULL THEN
...
sp_main: begin
    ...
    IF ip_airportID is not NULL THEN
...
sp_main: begin
    ...
    if ip_personID is not NULL THEN
...
sp_main: begin
    ...
    IF ip_routeID is not NUll and ip_flightID is not NULL THEN
...
sp_main: begin
    ...
    -- Ensure that the flight exists
    -- Ensure that the flight is on the ground
    IF (ip_flightID in (select flightID from flight)) and ('on_ground' = (select airplane_status from flight where flightID = ip_flightID)) THEN

		-- determine plane_type
		select plane_type into plane_type from airplane 
        ...
        -- determine required number of pilots
        IF plane_type = 'Boeing' THEN
			SET required_pilots = 2;
		ELSE
			SET required_pilots = 1;
		END IF;
        
		-- not enough pilots -> delay flight
		IF (select count(*) from pilot where commanding_flight = ip_flightID) < required_pilots THEN
		...
		ELSE
            ...
            IF curr_progress < (select max(sequence) from route_path join flight using(routeID) where flightID = ip_flightID) THEN
				...
				-- Calculate the flight time using the speed of airplane and distance of leg
				select distance into flight_distance from leg where legID = (select legID from route_path join flight using(routeID)
					where flightID = ip_flightID and sequence = progress);
				
                select speed into flight_speed from airplane join flight where tail_num = support_tail and airlineID = support_airline and flightID = ip_flightID;
				if flight_speed = 0 or flight_speed is null then
					set flight_speed = 1;
				end if;
	
				set flight_time = flight_distance / flight_speed;
	
				-- Update the next time using the flight time
				UPDATE flight set next_time = addtime(next_time, SEC_TO_TIME(flight_time * 3600)) where flightID = ip_flightID;
			END IF;
		END IF;
	END IF;
	...
	IF (ip_flightID in (select flightID from flight)) THEN
        ...
        IF ('on_ground' = (select airplane_status from flight where flightID = ip_flightID)) THEN
            ...
            IF ((select progress from flight where flightID = ip_flightID) < (select max(sequence) from route_path join flight using(routeID) where flightID = ip_flightID)) THEN
					...
					select departure into curr_airport from leg where legID = (select legID from route_path join flight using(routeID) where sequence = progress + 1 and flightID = ip_flightID);
					select arrival into next_airport from leg where legID = (select legID from route_path join flight using(routeID) where sequence = progress + 1 and flightID = ip_flightID);
                ...
                CREATE TEMPORARY TABLE temp_passengers AS
					select p.personID, n.funds from person p join passenger n using(personID) join passenger_vacations v USING(personID)
					where p.locationID = (select locationID from airport where airportID = curr_airport)
					and v.airportID = next_airport and v.sequence = 1 and n.funds >= flight_cost;
                
                -- Determine the number of passengers attempting to board the flight
				select COUNT(*) into passenger_count from temp_passengers;

				-- Check if there enough seats for all the passengers
				IF (passenger_count <= (select seat_capacity from flight join airplane on support_tail = tail_num and support_airline = airlineID where flightID = ip_flightID)) THEN
				
					-- If there are, deduct their funds
					UPDATE passenger n JOIN temp_passengers t ON n.personID = t.personID
                    SET n.funds = n.funds - flight_cost;
					
                    -- board them
					UPDATE person p JOIN temp_passengers t ON p.personID = t.personID
                    SET p.locationID = (SELECT locationID FROM airplane JOIN flight ON tail_num = support_tail
										AND airlineID = support_airline AND flightID = ip_flightID);
				END IF;
...
sp_main: begin
	...
	-- Use the following to check:
        ...
        -- Passenger has reached their immediate next destination airport
	...
	IF (ip_flightID in (select flightID from flight)) THEN
        ...
        IF ('on_ground' = (select airplane_status from flight where flightID = ip_flightID)) THEN
			...
			select arrival into arrival_airport from leg where legID = (select legID from route_path join flight using(routeID) where flightID = ip_flightID and sequence = progress) limit 1;
			select locationID into plane_location from flight join airplane on support_tail = tail_num and support_airline = airlineID where flightID = ip_flightID limit 1;
            
			DROP TEMPORARY TABLE IF EXISTS temp_passengers;
            
			CREATE TEMPORARY TABLE temp_passengers AS
				select * from person join passenger using(personID) join passenger_vacations using(personID) where airportID = arrival_airport
                and locationID = (select locationID from flight join airplane on support_tail = tail_num and support_airline = airlineID where flightID = ip_flightID);

			-- Move the appropriate passengers to the airport
            UPDATE person p JOIN temp_passengers t ON p.personID = t.personID
            SET p.locationID = (select locationID FROM airport where airportID = arrival_airport);

			-- Update the vacation plans of the passengers
            DELETE FROM passenger_vacations WHERE sequence = 1 AND personID IN (SELECT personID FROM temp_passengers);
            
            UPDATE passenger_vacations SET sequence = sequence - 1 WHERE sequence > 1 AND personID IN (SELECT personID FROM temp_passengers);
...
sp_main: begin
	...
	IF ip_flightID is not NULL and ip_personID is not NULL THEN
...
sp_main: begin
    ...
    -- Ensure that the flight is on the ground
    IF EXISTS (SELECT 1 FROM flight WHERE flightID = ip_flightID AND airplane_status = 'on_ground') THEN
        
        -- Ensure that the flight does not have any more legs
        IF (SELECT progress FROM flight WHERE flightID = ip_flightID) = (SELECT MAX(sequence) FROM route_path JOIN flight USING(routeID) WHERE flightID = ip_flightID) THEN
            
            -- Ensure that the flight is empty of passengers
            IF NOT EXISTS (SELECT 1 FROM passenger n JOIN person p ON n.personID = p.personID WHERE p.locationID =
					(SELECT locationID FROM airplane a WHERE a.airlineID = (SELECT support_airline FROM flight
                    WHERE flightID = ip_flightID) AND a.tail_num = (SELECT support_tail FROM flight WHERE flightID = ip_flightID))) THEN
			
                -- Move all pilots to the airport the plane of the flight is located at
                UPDATE person SET locationID = (
					select locationID from airport where airportID = (select arrival from leg where legID = (
					select legID from route_path rp join flight f using(routeID) where f.flightID = ip_flightID
					and rp.sequence = (select MAX(sequence) from route_path where routeID = f.routeID)
					)))
...
sp_main: begin
    ...
    IF EXISTS (SELECT 1 FROM flight WHERE flightID = ip_flightID AND airplane_status = 'on_ground') THEN
		...
		IF ((SELECT progress FROM flight WHERE flightID = ip_flightID) = 0)
				OR ((SELECT progress FROM flight WHERE flightID = ip_flightID) = (SELECT MAX(sequence)
                FROM route_path JOIN flight USING(routeID) WHERE flightID = ip_flightID)) THEN
    
			-- Ensure that there are no more people on the plane supporting the flight
			IF NOT EXISTS (SELECT 1 FROM person WHERE locationID = (SELECT locationID FROM flight f JOIN airplane a
            ON f.support_tail = a.tail_num WHERE flightID = ip_flightID)) THEN
...
sp_main: begin
    ...
    -- Identify the next flight to be processed
    DECLARE flight_to_be_processed varchar(50);
    SELECT flightId into flight_to_be_processed from flight order by next_time asc, (airplane_status = 'on_ground') asc, flightID asc limit 1;
    
    -- NULL check
    IF flight_to_be_processed IS NOT NULL THEN
    
		-- If the flight is in the air:
		IF (select airplane_status from flight where flightID = flight_to_be_processed) = 'in_flight' THEN
		
			-- Land the flight and disembark passengers
			call flight_landing(flight_to_be_processed);
			call passengers_disembark(flight_to_be_processed);
        
			-- If it has reached the end:
			IF (select progress from flight where flightID = flight_to_be_processed) =
					(select MAX(sequence) from route_path where routeID = (select routeID from flight where flightID = flight_to_be_processed)) THEN
				call recycle_crew(flight_to_be_processed);
				call retire_flight(flight_to_be_processed);
			END IF;
            
		-- If the flight is on the ground:
		ELSEIF (select airplane_status from flight where flightID = flight_to_be_processed) = 'on_ground' THEN
        
			-- Board passengers and have the plane takeoff
			call passengers_board(flight_to_be_processed);
			call flight_takeoff(flight_to_be_processed);
        END IF;
    END IF;
...
select city, state, country, count(*) as num_airports, GROUP_CONCAT(airportID order by airportID separator ',') as airport_code_list,
GROUP_CONCAT(airport_name order by airportID separator ',') as airport_name_list from airport group by city, state, country having count(*) > 1;
